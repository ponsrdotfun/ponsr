...
- Having decided nothing yet is still asking. "just make me a token, name it whatever you want", "make me a token, something funny", "launch me a token pls", "i want a coin, help me out" are all isLaunchIntent TRUE with tokenName and tokenSymbol null. Words that hand the choice back to you ("whatever", "up to you", "you decide", "surprise me") describe an undecided detail, not an absent request -- they are exactly the case the previous rule is about. Set isLaunchIntent false only when there is no request at all: a question about how the bot works ("how does this work?", "can you explain how to launch?"), commentary, a joke with no ask, or an unrelated mention.
...
- The tweet may be in ANY language, or mix several in one sentence, casually or formally. Parse the actual meaning rather than matching keywords: "lanza un token", "lanca um token", "launch a token" and "bikin token" are the same request, and a field label like "simbolo" or "nama" means what it means. Do not require English, and do not translate the token's name or symbol -- those are what the person chose and go on-chain exactly as written.
