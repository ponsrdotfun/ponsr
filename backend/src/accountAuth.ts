...
export class AccountAuthService {
  ...
  session(token?:string){if(!token)return{state:'unauthenticated' as const};const now=this.now(),row=this.db.getAccountSession(hashSecret(token),now.toISOString(),new Date(now.getTime()-30*60_000).toISOString());if(!row)return{state:'unauthenticated' as const};return{state:'authenticated' as const,session:{expiresAt:row.expires_at,idleTimeoutMinutes:30,csrfRequired:true},identity:{xUserId:row.x_user_id,handle:row.x_handle,verifiedAt:row.verified_at},wallet:{address:ethers.getAddress(row.wallet_address),provider:'privy',continuity:'existing-verified'},executionAuthority:'NONE_PREVIEW_ONLY',canSign:false,canSend:false,canSwap:false,canClaim:false};}
  ...
  launches(token?:string){const s:any=this.session(token);return s.state==='authenticated'?{state:'authenticated',launches:this.db.listLaunchesForUser(s.identity.xUserId),executionAuthority:'NONE_PREVIEW_ONLY'}:{state:'unauthenticated'};}
  wallet(token?:string){const s:any=this.session(token);return s.state==='authenticated'?{state:'authenticated',wallet:s.wallet,executionAuthority:'NONE_PREVIEW_ONLY',canSign:false,canSend:false,canSwap:false,canClaim:false}:{state:'unauthenticated'};}
}
