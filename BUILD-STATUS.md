...
| Smoke test | OK | **64/64 automated checks** (`website/smoke-test.js`) — routing by contract address, the grid, sorting/search/pagination, the what-if gate, motion, accessibility, the terms link, and that no asset path is relative. Several checks exist because a specific bug shipped: read the comment above one before changing it. |
