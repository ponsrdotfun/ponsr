...
| Bounded RPC fallback (`rpcPool.ts`) | OK (code) / EMPTY BY DEFAULT | **Added 2026-08-26.** A naive fallback is strictly worse than none: `FallbackProvider` and every hand-rolled retry loop assume the endpoints are interchangeable, so a testnet URL in slot two keeps the bot running against a different world while every guard answers confidently about somewhere else. Not hypothetical — `backend/.env` already holds a testnet URL, and on testnet this factory address holds no contract and the treasury has no funds. So each endpoint is **admitted before it may answer**: chain id must equal the deployment's, and the factory's runtime bytecode must hash to the registry's value, which also catches a fork or a lagging archive node. A refused endpoint is not "tried anyway with a warning". Failover is **bounded** — each endpoint at most once per operation, no retry loop, no backoff — because the caller is a check with a deadline. A wrong chain is remembered as refused; an outage is not, so an endpoint that was down at boot can return without a restart. `RPC_FALLBACK_URLS` is empty by default. **Wired to the read path only**: the launch path keeps its single pinned provider deliberately, since failing over mid-launch means a nonce reserved on one node and a broadcast through another, and the canary's ambiguity model assumes one view of the chain. 12 tests. |
| Identity watch (`identityWatch.ts`) | OK | **Added 2026-08-26.** The 48 KB bytecode check now runs on its own budget and its own cadence instead of ahead of every permission read inside the launchpad deadline — a slow file transfer must not publish a claim about pons's launchpad. It is **not weakened**: `assertDeploymentIdentity` still runs uncached on the launch path, before a splitter is deployed and before a fee is spent, and still throws. Three rules keep the cache honest: only a PASS is cached, a mismatch is re-read every time so a fix is noticed immediately and a real drift is never answered from memory; a failure to READ is not a pass and leaves the previous verdict standing with its true age; and the age is always published, so a remembered answer is never mistaken for a fresh one. Concurrent checks collapse into one download. 6 tests. |
