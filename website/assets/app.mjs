...
import { amountFromWei, curveFlowSeries, ethFromWei, eventTime, plural, quoteName, quoteUnit, reserveRows, shortAddress, whole } from './format.mjs';
...
function paintCurveActivity(token) {
  ...
  const head=element('div','curve-flow-head'),heading=element('div');heading.append(element('p','eyebrow','Cumulative quote accounting · authoritative block time'),element('h2','',`Net ${unit} flow`),element('p','flow-intro',`See how much ${quoteName(token)} entered through buys, exited through sells, and remained as net transaction flow.`));
  const summary=element('div',`flow-summary ${direction}`);summary.append(element('span','flow-not-price','Not token price · liquidity · PnL'),element('span','flow-summary-label',`Net ${direction}`),element('p','mono',amountFromWei(absNet,18,unit)));head.append(heading,summary);
  ...
  const rail=element('div','flow-metric-rail');for(const [key,label,value,note,cls=''] of [['flowBuyInflow','Buy inflow',`+${amountFromWei(buyWei,18,unit)}`,'Σ verified CurveBuy quote in'],['flowSellOutflow','Sell outflow',amountFromWei(sellWei,18,unit),'Unsigned magnitude · subtracted from buys'],['flowNetInflow',`Net ${direction}`,`${direction==='inflow'?'+':'−'}${amountFromWei(absNet,18,unit)}`,'Buy inflow − sell outflow',`net ${direction}`]]){const card=element('article',cls);card.dataset[key]='';card.append(element('span','',label),element('strong','',value),element('small','',note));rail.append(card);}
  const note=element('div','flow-ledger-note'),noteIcon=element('span','','◇');noteIcon.setAttribute('aria-hidden','true');const noteCopy=element('p');noteCopy.append(element('strong','','Transaction-flow ledger'),document.createTextNode(`Direction and cumulative ${unit} movement through exact curve events—not valuation or available reserve.`));note.append(noteIcon,noteCopy);
  const ns='http://www.w3.org/2000/svg',plot=element('div','flow-plot'),plotHead=element('div','flow-plot-head');plotHead.append(element('span','','Running net flow'),element('span','','Older events → newer events'));const maxWei=series.reduce((best,event)=>BigInt(event.netQuoteWei)>best?BigInt(event.netQuoteWei):best,0n);plot.append(plotHead,element('span','flow-scale flow-scale-max',amountFromWei(maxWei,18,unit)),element('span','flow-scale flow-scale-zero',`0 ${unit}`));
...
function paintTokenPage(feed) {
  ...
  if(reserves&&rows){const values=Object.fromEntries(rows),grid=element('div','reserve-metric-grid');for(const [label,note] of [['Real quote reserve',`Observed ${quoteName(token)}`],['Graduation threshold','Protocol threshold']]){const card=element('article');card.append(element('span','',label),element('strong','',values[label]),element('small','',note));grid.append(card);}const state=element('div','curve-state-row');state.append(element('span','','Curve status'),element('strong','',values['Curve status']),element('small','',`Observed at ${values['Observed at']}`));reserves.replaceChildren(grid,state,element('p','observation-note','A single reading, not a live figure. It was true at the observation above, and it is not extrapolated anywhere on this page.'));}
...
async function runSimulator(host) {
  const input=host.querySelector('[data-wallet-input]');const wallet=String(input?.value||'').trim();const token=host.dataset.token,quote=(host.dataset.quoteUnit||'').trim(),withQuote=(value)=>quote?`${value} ${quote}`:value;
